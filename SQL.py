...
def khoi_tao_csdl():
    ...
    sinh_vien_mau = [
         ('Trần Minh Khôi', 'Công nghệ thông tin - CLC', round(random.uniform(1.0, 4.0), 1)),
         ('Phạm Thảo Nguyên', 'Công nghệ thông tin - CLC', round(random.uniform(1.0, 4.0), 1)),
         ('Hoàng Đức Anh', 'Công nghệ thông tin - CLC', round(random.uniform(1.0, 4.0), 1)),
         ('Nguyễn Thu Hà', 'Công nghệ thông tin - CLC', round(random.uniform(1.0, 4.0), 1)),
         ('Lê Quang Huy', 'Công nghệ thông tin - CLC', round(random.uniform(1.0, 4.0), 1)),
         ('Đặng Thị Mai', 'Công nghệ thông tin - CLC', round(random.uniform(1.0, 4.0), 1)),
          ('Nguyễn Văn Phúc', 'Công nghệ thông tin - CLC', round(random.uniform(1.0, 4.0), 1)),
          ('Bùi Thị Hạnh', 'Công nghệ thông tin - CLC', round(random.uniform(1.0, 4.0), 1)),
         ('Vũ Hoàng Nam', 'Công nghệ thông tin - CLC', round(random.uniform(1.0, 4.0), 1)),
         ('Phan Thị Lan', 'Công nghệ thông tin - CLC', round(random.uniform(1.0, 4.0), 1))
...
class UngDungDaiHoc:
    def __init__(self, cua_so_chinh):
        ...
        self.cua_so_chinh.configure(bg="#e8f5e9")  # màu nền tổng thể

        khung_nhap_lieu = tk.LabelFrame(self.cua_so_chinh, text="Thông tin Sinh viên", bg="#c8e6c9", fg="#1b5e20")
        khung_nhap_lieu.pack(fill="x", padx=10, pady=10)

        tk.Label(khung_nhap_lieu, text="Tên (Name):", bg="#c8e6c9").grid(row=0, column=0, padx=5, pady=5)
        self.o_nhap_ten = tk.Entry(khung_nhap_lieu, width=25, bg="#ffffff", fg="#2e7d32")
        self.o_nhap_ten.grid(row=0, column=1, padx=5, pady=5)

        tk.Label(khung_nhap_lieu, text="Ngành (Major):", bg="#c8e6c9").grid(row=0, column=2, padx=5, pady=5)
        self.o_nhap_nganh = tk.Entry(khung_nhap_lieu, width=25, bg="#ffffff", fg="#2e7d32")
        self.o_nhap_nganh.grid(row=0, column=3, padx=5, pady=5)

        tk.Label(khung_nhap_lieu, text="GPA:", bg="#c8e6c9").grid(row=1, column=0, padx=5, pady=5)
        self.o_nhap_gpa = tk.Entry(khung_nhap_lieu, width=25, bg="#ffffff", fg="#2e7d32")
        self.o_nhap_gpa.grid(row=1, column=1, padx=5, pady=5)

        nut_them = tk.Button(khung_nhap_lieu, text="Thêm Sinh viên", command=self.them_sinh_vien,
                             bg="#388e3c", fg="white", activebackground="#1b5e20")
        nut_them.grid(row=1, column=2, columnspan=2, pady=5, sticky="we")

        khung_bang = tk.Frame(self.cua_so_chinh, bg="#e8f5e9")
        ...
        khung_chuc_nang = tk.LabelFrame(self.cua_so_chinh, text="Chức năng", bg="#f1f8e9", fg="#33691e")
        khung_chuc_nang.pack(fill="x", padx=10, pady=10)

        tk.Button(khung_chuc_nang, text="Tất cả SV", command=self.tai_tat_ca, bg="#aed581").grid(row=0, column=0, padx=5, pady=5)
        tk.Button(khung_chuc_nang, text="Lấy SV có GPA > 3.0", command=self.tai_gpa_lon_hon_3, bg="#81c784").grid(row=0, column=1, padx=5, pady=5)
        tk.Button(khung_chuc_nang, text="Cập nhật GPA", command=self.cap_nhat_gpa, bg="#4caf50", fg="white").grid(row=0, column=2, padx=5, pady=5)
        tk.Button(khung_chuc_nang, text="Xóa SV", command=self.xoa_sinh_vien, bg="#ffb74d").grid(row=0, column=3, padx=5, pady=5)
        tk.Button(khung_chuc_nang, text="Xóa tất cả SV GPA < 2.0", command=self.xoa_gpa_nho_hon_2, bg="#e57373", fg="white").grid(row=0, column=4, padx=5, pady=5)
